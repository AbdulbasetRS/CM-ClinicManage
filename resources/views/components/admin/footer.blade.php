<footer class=" border-top bg-light mt-5">
    <div class="container py-3 d-flex align-items-center justify-content-between">
        ...
        <div class="text-muted small d-flex flex-column align-items-end">
            <div class="mb-1">
                ...
                <span> - {{ \App\Helpers\DateHelper::getUserTimezone() }}</span>
            </div>
    ...
    <script>
        (function() {
            ...
            // ============================================
            // Server Time Counter - عداد وقت السيرفر
            // ============================================
            // نبدأ من الوقت اللي جاي من السيرفر: {{ now()->format('Y-m-d H:i:s') }}
            // ونزود عليه ثانية كل ثانية

            const serverTimeString =
                "{{ \App\Helpers\DateHelper::convertAndFormat(now(), \App\Helpers\DateHelper::getUserTimezone(), 'Y-m-d H:i:s') }}";
            // const serverTimeString = '{{ now()->format('Y-m-d H:i:s') }}';
            let serverDate = new Date(serverTimeString.replace(' ', 'T')); // تحويل الوقت لـ Date object
            ...
            function updateServerTime() {
                ...
                // Get formats from PHP helper
                const userDateFormat = '{{ \App\Helpers\DateHelper::getUserDateFormat() }}';
                const userTimeFormat = '{{ \App\Helpers\DateHelper::getUserTimeFormat() }}';
                ...
                // نعرض الوقت
                const timeString = serverDate.toLocaleString(displayLocale, options);
