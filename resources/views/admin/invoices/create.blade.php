...
@section('title', __('admin.create_new_invoice'))
                ...
                <div class="col-12">
                    ...
                    <div class="card mb-3">
                        <div class="card-header">
                            <h3 class="card-title mb-0"><i class="fas fa-plus"></i> {{ __('admin.create_new_invoice') }}
                            </h3>
                        </div>
                    </div>
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-control-plaintext">
                                            @if (isset($patient))
                                                <a href="{{ route('admin.users.show', $patient->slug) }}" target="_blank">
                                                    {{ $patient->username }} <i class="fas fa-external-link-alt small"></i>
                                                </a>
                                                <input type="hidden" name="patient_id" value="{{ $patient->id }}">
                                            @else
                                                {{-- هنا يمكن إضافة Select لاختيار المريض إذا لم يكن محدداً --}}
                                                <select class="form-select" name="patient_id" required>
                                                    <option value="">{{ __('admin.select') }}
                                                        {{ __('admin.patient') }}...</option>
                                                    @foreach (\App\Models\User::where('type', 'patient')->get() as $p)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-control-plaintext">
                                            @if (isset($visit))
                                                <a href="{{ route('admin.visits.show', $visit->id) }}" target="_blank">
                                                    {{ __('admin.visit') }} #{{ $visit->id }} <i
                                                        class="fas fa-external-link-alt small"></i>
                                                </a>
                                                <input type="hidden" name="visit_id" value="{{ $visit->id }}">
                                                {{-- إذا كان هناك زيارة، فالمريض هو مريض الزيارة --}}
                                                @if (!isset($patient))
                                                ...
                                                <tr class="item-row">
                                                    <td>
                                                        <input type="text" class="form-control"
                                                            ...
                                                            placeholder="وصف الخدمة أو العلاج">
                                                    </td>
                                                    <td>
                                            ...
                                            <tr class="item-row">
                                                ...
                                                <td>
                                                    <input type="number" step="0.01" class="form-control item-price"
                                                        name="items[0][amount]" value="0" min="0" required>
                                                </td>
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-danger btn-sm remove-item">
                                                        <i class="fas fa-trash"></i>
                    ...
                    <div class="card-footer d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> {{ __('admin.save_invoice') }}
                        </button>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function attachEvents() {
                ...
                document.querySelectorAll('.remove-item').forEach(btn => {
                    // Remove existing listeners to avoid duplicates if called multiple times (though here it's fine)
                    // Better to use event delegation for delete buttons too, but keeping consistent with existing style
